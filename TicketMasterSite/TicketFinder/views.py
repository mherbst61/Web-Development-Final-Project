...
def SavedTicket(request):
    if request.method == "POST":
        ...
        if SavedTickets.objects.filter(ticket_id=ticket_id).exists():
            ...
            SavedTickets.objects.create(event_name=event_name, event_url=event_url, theater_name=theater_name,
                                        theater_address1=theater_address1, theater_address2=theater_address2,
                                        event_imageUrl=event_imageUrl, event_Date=event_Date, event_Time=event_Time,
                                        ticket_id=ticket_id)
